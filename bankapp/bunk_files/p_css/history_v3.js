...
//     for (let i = 0; i < sampleData.summaryByDate.length; i++) {
//         fileData[0].graphData[0].column[9 - i].data = sampleData.summaryByDate[i].totalAmount;
//         console.log(fileData[0].graphData[0].column[9 - i].data);
//     }
//     for (let i = 0; i < sampleData.summaryByMonth.length; i++) {
//         fileData[0].graphData[1].column[9 - i].data = sampleData.summaryByMonth[i].totalAmount;
//         console.log(fileData[0].graphData[1].column[9 - i].data);
//     }

//     for (let i = 0; i < 20; i++) {
//         if (i == sampleData.rawData.length) {
//             break;
//         }
//         transData.push(sampleData.rawData[i]);
//     }
// })
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    const transactionDeployUrl = setQueryParams(idDeployUrl, transactionParams);
    fetch(transactionDeployUrl).then(function (transactionResponse) {
...
function setHistory() {
    ...
    if (transData) {
        historyArea.appendChild(makeHistoryList(transData));
    }
}
...
function makeHistoryList(data) {
    if (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const yearValue = data[i].Timestamp.substr(0, 4);
            const monthValue = data[i].Timestamp.substr(5, 2);
            const dayValue = data[i].Timestamp.substr(8, 2);
            const dateValue = yearValue + "年" + monthValue + "月" + dayValue + "日";
            ...
            const hourValue = data[i].Timestamp.substr(11, 2);
            const minuteValue = data[i].Timestamp.substr(14, 2);
...
window.addEventListener('DOMContentLoaded', penguinSwitch);

// ペンギン出し分け
function penguinSwitch() {
    graphMenu = document.getElementsByClassName('bh_pullDown_graphSelector_select')[0];
